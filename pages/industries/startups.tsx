...
import type { StartupsData } from 'types/pages'
import { getI18nStaticProps } from 'utils/i18n-ssr'

interface Props {
  rawData: StartupsData & { footerData: FooterProps }
}

export const getStaticProps = getI18nStaticProps('industries/startups', ['startups'])

const Startups: NextPage<Props> = ({ rawData }) => {
  const { footerData, ...pageData } = rawData
  const footerProps = useTransformedData(footerData, 'footer')
  const { headerProps, heroProps, challengesProps, serviceProps, ctaProps } = useTransformedData(pageData, 'startups')
