...
import type { HealthcareData } from 'types/pages'
import { getI18nStaticProps } from 'utils/i18n-ssr'

interface Props {
  rawData: HealthcareData & { footerData: FooterProps }
}

export const getStaticProps = getI18nStaticProps('industries/healthcare', ['healthcare'])

const Healthcare: NextPage<Props> = ({ rawData }) => {
  const { footerData, ...pageData } = rawData
  const footerProps = useTransformedData(footerData, 'footer')
  const { headerProps, heroProps, challengesProps, serviceProps, ctaProps } = useTransformedData(pageData, 'healthcare')
